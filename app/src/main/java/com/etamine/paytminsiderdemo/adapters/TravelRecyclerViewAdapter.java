...
public class TravelRecyclerViewAdapter extends RecyclerView.Adapter<TravelRecyclerViewAdapter.MyViewHolder> {
    ...
    public TravelRecyclerViewAdapter(Context context, ArrayList<MasterList> masterList){
    ...
    @NonNull
    @Override
    public TravelRecyclerViewAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(context).inflate(R.layout.itemviewevents, parent, false);
        return (new MyViewHolder(itemView));
    }

    @Override
    public void onBindViewHolder(@NonNull TravelRecyclerViewAdapter.MyViewHolder holder, int position) {
        MasterList currentData = masterList.get(position);

        if (currentData.getGroup_id().getName().equalsIgnoreCase("travel")) {
