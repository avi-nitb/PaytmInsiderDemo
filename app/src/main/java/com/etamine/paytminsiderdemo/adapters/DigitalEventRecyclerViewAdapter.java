...
public class DigitalEventRecyclerViewAdapter extends RecyclerView.Adapter<DigitalEventRecyclerViewAdapter.MyViewHolder> {

    private Context context;
    private ArrayList<MasterList> masterList;

    public DigitalEventRecyclerViewAdapter(Context context, ArrayList<MasterList> masterList){
    ...
    @NonNull
    @Override
    public DigitalEventRecyclerViewAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(context).inflate(R.layout.itemviewevents, parent, false);
        return (new MyViewHolder(itemView));
    }

    @Override
    public void onBindViewHolder(@NonNull DigitalEventRecyclerViewAdapter.MyViewHolder holder, int position) {
        MasterList currentData = masterList.get(position);

        if (currentData.getGroup_id().getName().equalsIgnoreCase("Digital Events")) {
